function data = thr1F_run(thr1F,aio_s,trig_s, csvLoad, p_data)
...
%   trig_s          - daq session for sending external trigger signals
%   csvLoad         - csv file of conditions
% % Output variables %
...
btn = string(csvLoad.thr1F_button);
...
switch btn
    ...
    otherwise
        error('Unknown button mapping: %s', thr1F.button);
end
...
switch data.resp_btn_map
    ...
    case {2}
        data.resp_txt = flipud(thr1F.resp_txt); % flip text order
...
% instr_dir = fullfile(fileparts(mfilename('fullpath')), thr1F.instr_dir);
% instr_subdir = dir(fullfile(instr_dir, thr1F.instr_subdir_wildcard));
% 
% % Load image data
...
if strcmp(thr1F.button,'YN')
   instr_images = load_images(fullfile(instr_dir, 'condition_1'), thr1F.instr_img_wildcard);

elseif strcmp(thr1F.button,'NY')
    instr_images = load_images(fullfile(instr_dir, 'condition_3'), thr1F.instr_img_wildcard);

else
    error('Instruction images error: unknown button mapping %s', thr1F.button);
end

% Show images
[data.btn_instr, data.onset_instr] = show_instr_img_thr1F(instr_images, window, thr1F);
...
for i = 1:size(data.seq,1)
    ...
    send_pportTrigger(io64, hex2dec('D010'), thr1F.fixOnset);
    ...
    send_pportTrigger(io64, hex2dec('D010'), thr1F.cueOnset);
    %%% STIMULUS %%%
   ...
   data.ao_cue_post{i,1} = GetSecs; %%is itnot actually trigger post?
    ...
    [data.resp_btn(i,1),data.resp_t(i,1)] = register_button_press(data.onset_resp{i,1}, thr1F.maxRespTime,thr1F.btn, thr1F.s_port, thr1F.lpt_adr1 ,thr1F.trigger_length);
...
%             break
    ...
    if data.seq(i) == -1
        ...
        if UD.stop
            ...
            tmp_stim_range = min(tmp_priorAlphaRange):thr1F.priorAlphaSteps:max(tmp_priorAlphaRange);  % LIOBA NEW (05.02.2026)
