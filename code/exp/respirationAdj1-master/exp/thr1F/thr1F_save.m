...
% Write header
fprintf(data_file,'ID\tage\tgender\tcapsize\tsurveyid\tdate\tblock\ttrial\tstim_type\tintensity\tresp\tresp_t\tresp_btn\tstim_delay\tfix_t\tao_error\n');

for i = 1:length(d.seq)
   fprintf(data_file,'%s\t%s\t%s\t%s\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f\t%.6f\t%.0f\t%.4f\t%.0f\t%.6f\t%.6f\t%.6f\n',p_data.ID,p_data.age_years,p_data.gender_fmd,p_data.capsize_cm, p_data.surveyid_soscisurvey,date_str,d.block(i),i,d.seq(i,1),d.intensity(i),d.resp(i),d.resp_t(i),d.resp_btn(i),d.stim_delay(i),d.fix_t(i),d.ao_error(i,1));
end
