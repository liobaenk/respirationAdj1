...
%
% This function builds on io64.mexw64 and inpoutx64.dll (for installation
% details see http://apps.usd.edu/coglab/psyc770/IO64.html)
%
...
%% Subfunctions
lpt.get = @(dio,adr) uint8(io64(dio,adr));

lpt.set_mode = @(lpt,mode) io64(lpt.dio,lpt.ecr,set_bits(lpt.get(lpt.dio,lpt.ecr),lpt.mode_bits,mode));

lpt.set_bibit = @(lpt,bibit) io64(lpt.dio,lpt.ctrl_adr,bitset(lpt.get(lpt.dio,lpt.ctrl_adr),lpt.bidir_bit,bibit));

lpt.get_bibit = @(lpt) bitget(lpt.get(lpt.dio,lpt.ctrl_adr),lpt.bidir_bit);

lpt.set_IRQ = @(lpt,IRQbit) io64(lpt.dio,lpt.ctrl_adr,bitset(lpt.get(lpt.dio,lpt.ctrl_adr),lpt.IRQ_bit,IRQbit));

%% Create io64 interface
clear io64;
lpt.dio = io64;

%% Install inpoutx64.dll driver

status = io64(lpt.dio);

if status == 0
    disp('Successful installation of inpoutx64.dll'); 
else
    error('Failed installation of inpoutx64.dll');
end
