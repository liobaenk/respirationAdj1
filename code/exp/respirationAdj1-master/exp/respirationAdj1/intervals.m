...
%% SCREEN ONSET INTERVALS
    ...
    data.t_cue_stim(i,1) = ( data.ao_trigger_pre(i,1)-data.onset_cue{i,1})*1000; % fix to cue interval
    data.t_stim_resp1(i,1) = (data.onset_resp1{i,1}- data.ao_trigger_post(i,1))*1000; % stim to resp1 interval
...
%% CUE TO AO TRIGGER
   ...
   data.t_cue_ao_trigger_pre_diff_stim_delay(i,1) = (data.ao_trigger_pre(i,1)-data.onset_cue{i,1}-data.seq(i,3))*1000;  
