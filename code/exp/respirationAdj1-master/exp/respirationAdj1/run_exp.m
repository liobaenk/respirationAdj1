...
%% Response-button mapping (from CSV)

respOrder = string(csvLoad.button_response_order);

% Depending on csv button response
switch respOrder
    case "YNYN"
        data.resp_btn_map = 1;
    case "YNNY"
    ...
    case "NYNY"
        data.resp_btn_map = 4;
    otherwise
        error('Button mapping error: %s', respOrder);
end
...
% % Start trial is exception  in here
% currentBlock = blockList(block);
% if strcmp(currentBlock,'RhyF')
%     num_startTr = s.RhyF.start_trials_per_block;
% elseif strcmp(currentBlock,'RanS')
%     num_startTr = s.RanS.start_trials_per_block;
% elseif strcmp(currentBlock,'RhyS')
%     num_startTr = s.RhyS.start_trials_per_block;
% end
% 
% data.seq(num_startTr+1:end,4) = data.seq(num_startTr+1:end,4) - mod(data.seq(num_startTr+1:end,4), flip_t);
...
%data.trial_t = cond.fix_t + s.cue_t + s.resp1_max_t + s.resp2_max_t;
%con.fix_t will not work here since it is not one value
...
% Update sequence
data.fix_t = data.seq(:,4);
% Make fix_t divisible by flip time
data.fixFlip = round(data.fix_t / data.flip_t);

% Blank screen serving as the cue (cue_t)
data.cue_t = s.cue_t; % Make cue_t divisible by flip time
data.cueFlip = round(data.cue_t / data.flip_t);
%  data.onset_stim_p,...
%  data.onset_ITI
...
% Show images
[data.btn_instr, data.onset_instr] = show_instr_img(instr_images,window,s);
...
for i = 1:size(data.seq,1)    
    ...
    % Set font size for symbols, this part lasi makes fix sign to shown
    % between 2 responses 
    ...
    %% Send EEG trigger code for fix start
   
    send_pportTrigger(io64, hex2dec('D010'), 10); 
    ...
    %[data.onset_cue{i,:}] = Screen('Flip',window,data.onset_fix{i,1} + data.seq(i,4) - flip_t);
    
    [data.onset_cue{i,:}] = Screen('Flip',window,data.onset_fix{i,1} + data.fixFlip(i) * data.flip_t - data.flip_t); 
    
    
    %[data.onset_cue{i,:}] = Screen('Flip', window,data.onset_fix{i,1} + data.fixFlip(i) * data.flip_t);
    %% Send EEG trigger code for cue start
    send_pportTrigger(io64, hex2dec('D010'), s.cueOnset);
    ...
    switch data.seq(i,2)
        ...
        case 1 % near
            data.intensity(i,1) = round_dec(data.near,s.stim_dec);
        %case 2 % supra
    ...
    %data.ao_trigger_pre(i,1) = WaitSecs('UntilTime',data.onset_cue{i,1} + data.seq(i,4) - (s.pre_pulse_t/1000));
        ...
        %data.ao_trigger_pre(i,1) = WaitSecs('UntilTime',data.onset_cue{i,1} + data.seq(i,3) - (s.pre_pulse_t/1000));
        data.ao_trigger_pre(i,1) = WaitSecs('UntilTime',data.onset_cue{i,1} + data.seq(i,3) - (s.pre_pulse_t/1000));
...
%             stop(aio_s);
   ...
   %data.ao_cue_post{i,1} = GetSecs; %%is itnot actually trigger post?
    data.ao_trigger_post(i,1) = GetSecs; % TODO: save this and pre stim in the txt file, Rosa did it like this
    
    %% For the stimulus onset 
    
    %bu olmaysa tam stimonset kullan [data.onset_stim{i,:}] = Screen('Flip',window); %maybe this will be a less accurate timing of stimulus onset...

            %send_pportTrigger(io64, hex2dec('D010'), s.stimOnset);
         %DrawFormattedText(window,s.cue,'center','center',s.cue_color);
        % [data.onset_stim{i,:}] = Screen('Flip',window);
        %[data.onset_stim{i,:}] = Screen('Flip',window,data.onset_cue{i,1} + 0.7);
        %[data.onset_stim{i,:}] = Screen('Flip', window, data.onset_cue{i,1} + s.stim_delay(1));
       
%            stimFlip = round(data.seq(i,3)/ data.flip_t);
%          [data.onset_stim{i,:}] = Screen('Flip', window, data.onset_cue{i,1} + stimFlip * data.flip_t);
           %[data.onset_stim{i,:}] = Screen('Flip', window);
        %send_pportTrigger(io64, hex2dec('D010'), s.stimOnset); %50 = stimulus onset
%          
   ...
   % [data.onset_resp1{i,:}] = Screen('Flip',window,data.onset_cue{i,1} + s.cue_t - flip_t);
    ...
    [data.onset_resp1{i,:}] = Screen('Flip',window,data.onset_cue{i,1} + data.cueFlip * data.flip_t - data.flip_t);
 %[data.onset_resp1{i,:}] = Screen('Flip', window, data.onset_cue{i,1} + data.cueFlip * data.flip_t);
...
%     [data.resp1_btn(i,1),data.resp1_t(i,1),data.resp1_port(i,:)] = parallel_button(s.resp1_max_t,data.onset_resp1{i,1},s.resp_window,s.debounceDelay,lpt);        
    [data.resp1_btn(i,1),data.resp1_t(i,1)] = register_button_press(data.onset_resp1{i,1},s.resp1_max_t,s.btn_resp1, s.s_port, s.lpt_adr1 ,s.trigger_length);
    
    % RT dependent fix between responses, we don`t have a pause
...
%      [data.onset_resp2{i,:}] = Screen('Flip',window,data.onset_resp1{i,1}); 
     [data.onset_resp2{i,:}] = Screen('Flip',window,data.onset_resp1{i,1} + data.resp1_t(i,1) - data.flip_t);
...
%     [data.resp2_btn(i,1),data.resp2_t(i,1),data.resp2_port(i,:)] = parallel_button(s.resp2_max_t,data.onset_resp2{i,1},s.resp_window,s.debounceDelay,lpt);   
    [data.resp2_btn(i,1),data.resp2_t(i,1)] = register_button_press(data.onset_resp2{i,1},s.resp2_max_t,s.btn_resp2, s.s_port, s.lpt_adr1 ,s.trigger_length);
