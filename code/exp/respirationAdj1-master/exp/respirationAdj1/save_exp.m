...
% Write header
fprintf(data_file,'ID\tage\tgender\tcapsize\tsurveyid\tdate\tblock\ttrial\tstim_type\tstim_step\tintensity\tresp1\tresp1_t\tresp1_btn\tresp2\tresp2_t\tresp2_btn\tstim_delay\tfix_t\tonset_fix\tonset_cue\tonset_resp1\tonset_resp1_p\tonset_resp2\tstim_onset\tao_error\n');

for i = 1:length(d.seq)
   fprintf(data_file,'%s\t%s\t%s\t%s\t%.0f\t%.0f\t%.0f\t%.2f\t%.6f\t%.0f\t%.4f\t%.0f\t%.0f\t%.4f\t%.0f\t%.4f\t%.4f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.0f\n',p_data.ID,p_data.age_years,p_data.gender_fmd,p_data.capsize_cm, p_data.surveyid_soscisurvey, date_str,d.seq(i,1),i,d.seq(i,2),d.seq(i,3),d.intensity(i),d.resp1(i),d.resp1_t(i),d.resp1_btn(i),d.resp2(i),d.resp2_t(i),d.resp2_btn(i),d.seq(i,4),d.seq(i,5),d.onset_fix{i,1},d.onset_cue{i,1},d.onset_resp1{i,1},d.onset_resp2{i,1},d.t_fix_stim_onset(i,1),d.ao_error(i,1));
end
